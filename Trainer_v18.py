...
from models.PGM import VAEPrior
...
def get_H_C_P(cfg):
    ...
    C = len(cfg.features) if hasattr(cfg, "features") and isinstance(cfg.features, (list, tuple)) \
        else getattr(cfg, "n_features", None) or getattr(cfg, "C", None)
...
def main():
    ...
    for ep in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_pbar, start=1):
            delta_norm = batch['Δ_norm'].to(device).float()
            y = delta_norm[:, -H:, :]  # [B,H,C]
            w = get_phase_w_mixed(
            ...
            with torch.cuda.amp.autocast(enabled=scaler.is_enabled()):
                _, _, _, _, loss, info = vae(y, w, compute_elbo=True, beta=beta, reduction="mean")
        ...
        if eval_loader is not None:
            ...
            with torch.no_grad():
                for batch in eval_pbar:
                    ...
                    _, _, _, _, loss, info = vae(y, w, compute_elbo=True, beta=1.0, reduction="mean")
                    ev_loss += float(loss)
