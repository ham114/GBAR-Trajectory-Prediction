...
def main():
    ...
    for ep in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_pbar, start=1):
            ...
            avg_kl = tr_kl / step
            train_pbar.set_postfix(loss=f"{float(loss):.8f}",
                                   avg_loss=f"{avg_loss:.8f}",
                                   recon=f"{float(info['recon']):.8f}",
                                   kl=f"{float(info['kl']):.8f}",
                                   beta=f"{beta:.8f}")

        n_train = len(train_loader)
        train_msg = (f"[Epoch {ep:02d}] train "
                     f"loss={tr_loss / n_train:.8f}  recon={tr_recon / n_train:.8f}  kl={tr_kl / n_train:.8f}  "
        ...
        if eval_loader is not None:
            ...
            with torch.no_grad():
                for batch in eval_pbar:
                    ...
                    _, loss, info = vae(y, w, mode="elbo", beta=1.0, reduction="mean")
                    ev_loss += float(loss)
