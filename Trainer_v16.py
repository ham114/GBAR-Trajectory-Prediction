...
import datetime
import json
from load_data.dataloader_v14 import DataGenerator
from models.GeoHashBARTNet_v13 import GeohashBART
...
class TrainClient():
    ...
    def run_train_epoch(self, epoch, batch_size=1024):
        ...
        for i, batch in enumerate(tq):
            ...
            raw_data_norm = batch['raw_data_norm'].float()  # [B, inp_len + horizon, C]
            diff_data_norm = batch['diff_data_norm'].float()

            en_inputs = raw_data_norm[:, :self.configs.inp_seq_len, :]  # 模型输入
            de_inputs = diff_data_norm[:, :self.configs.inp_seq_len - 1, :]

            target = diff_data_norm[:, -self.configs.horizon:, :]  # 预测目标

            with autocast('cuda'):
                pred = self.model(en_inputs, de_inputs)  # [B, H, C]
    ...
    def run_test(self, epoch, batch_size=64, full_batch=False):
        ...
        # === 获取差分归一化因子 ===
        diff_stat = self.eval_gen.data_dict['diff_stat'].to(device)  # [C,2]
        mins = diff_stat[:, 0].view(1, 1, -1)
        maxs = diff_stat[:, 1].view(1, 1, -1)
        ...
        with torch.inference_mode():
            for batch in tqdm(loader, desc="Eval", unit="batch"):
                ...
                raw_data_norm = batch['raw_data_norm'].float().to(device)
                diff_data_norm = batch['diff_data_norm'].float().to(device)
                raw_data = batch['raw_data'].float().to(device)

                en_inputs = raw_data_norm[:, :self.configs.inp_seq_len, :]
                de_inputs = diff_data_norm[:, :self.configs.inp_seq_len - 1, :]
                target = diff_data_norm[:, -self.configs.horizon:, :]
                ...
                # === 逆归一化预测差分 ===
                pred_diff = pred * (maxs - mins) + mins  # [B, H, C]

                # === 累加恢复真实值 ===
                last_obs = raw_obs[:, -1:, :]  # [B, 1, C]
                pred_real = []
                cur = last_obs.clone()
                for t in range(pred_diff.shape[1]):
                    cur = cur + pred_diff[:, t:t + 1, :]
                    pred_real.append(cur)
                pred_real = torch.cat(pred_real, dim=1)  # [B, H, C]
        ...
        if not self.configs.is_training:
            ...
            torch.save({
                ...
                "all_cls": torch.cat(all_cls_list, dim=0).cpu() if all_cls_list else None,
                "feat_names": feat_names
    ...
    def evaluate_predictions(
        ...
        # === 读取差分归一化因子（从 eval_gen 中读取）===
        diff_stat = self.eval_gen.data_dict['diff_stat'].to(device)  # shape: [C, 2]
        mins = diff_stat[:, 0].view(1, 1, -1)  # shape: [1,1,C]
        maxs = diff_stat[:, 1].view(1, 1, -1)

        # === 模型输出逆归一化 ===
        pred_diff = pred_normed * (maxs - mins) + mins  # [B, H, C]




        # === 利用 raw_obs 逐步重构预测轨迹 ===
        last_obs = raw_obs[:, -1:, :]  # 观测段最后一个时间步 [B,1,C]
        pred_reconstructed = []
        cur = last_obs.clone()

        for t in range(pred_diff.shape[1]):
            cur = cur + pred_diff[:, t:t + 1, :]  # 按时间逐步加差分
            pred_reconstructed.append(cur)

        pred_real = torch.cat(pred_reconstructed, dim=1)  # shape: [B, H, C]

        # === 拼接为完整轨迹 ===
        reconstructed_pred = torch.cat([raw_obs, pred_real], dim=1)
        ...
        dH = (pred_real[..., idx_R] - target_real[..., idx_R]) / 1000  # 转换单位为 km
        mde = torch.sqrt(dE ** 2 + dN ** 2 + dH ** 2).mean().item()

        if epoch == 0 or epoch == 1:
            print("==== DEBUG SAMPLE ====")
            print("Raw last obs:", last_obs[0, 0, :3])
            print("Pred diff mean/std:", pred_diff[0, :, :3].mean(0), pred_diff[0, :, :3].std(0))
            print("Pred real last:", pred_real[0, -1, :3])
            print("Target real last:", target_real[0, -1, :3])
            print("dE/dN/dR:", dE[0, -1].item(), dN[0, -1].item(), dH[0, -1].item())
