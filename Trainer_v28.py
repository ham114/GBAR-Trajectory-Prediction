...
default_config = 'config_v21.json'
...
class TrainClient():
    ...
    def evaluate_predictions(
        ...
        # === 拼接完整轨迹（观测段 + 预测段） ===
        reconstructed_pred = torch.cat(
            [raw_obs.to(device).float() / scale, pred_real], dim=1
        )  # [B, T_obs+H, C]
        reconstructed_true = torch.cat(
            [raw_obs.to(device).float() / scale, true_real], dim=1
        )
        ...
        # =====================================================
        # MDE：用纬度 / 经度计算大圆距离，单位 km
        # 现在约定：前三个特征为 [lat, lon, alt]，lat/lon 为「度」
        # =====================================================
        idx_lat, idx_lon = 0, 1  # 按你现在的数据格式，前两维是纬度、经度

        lat_true = true_real[..., idx_lat]   # [B, H]
        lon_true = true_real[..., idx_lon]
        lat_pred = pred_real[..., idx_lat]
        lon_pred = pred_real[..., idx_lon]
        ...
        a = torch.sin(dlat / 2) ** 2 + \
            torch.cos(lat_true_rad) * torch.cos(lat_pred_rad) * torch.sin(dlon / 2) ** 2
        c = 2 * torch.arcsin(torch.clamp(torch.sqrt(a), max=1.0))

        R_earth = 6371.0  # km
        dist = R_earth * c  # [B, H]，每个时间步的球面距离，单位 km
