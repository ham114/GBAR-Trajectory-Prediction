...
class CustomBalancedBatchSampler(Sampler):
    def __init__(self, labels, batch_size, rare_class=3, rare_ratio=0.1):
        self.labels = labels
...
def get_dataloaders(data_path, batch_size=128, val_split=0.1):
    data = torch.load(data_path)
    X, y = data["X_bin"], data["y_cls"]
    labels = y.argmax(dim=-1)

    train_idx, val_idx = train_test_split(
        np.arange(len(X)), test_size=val_split, stratify=labels, random_state=42
    )

    train_dataset = FlightDataset(X[train_idx], y[train_idx])
    val_dataset = FlightDataset(X[val_idx], y[val_idx])
...
def train():
    ...
    model = MambaClassifier(config).to(device)
    model.get_num_params()
    train_loader, val_loader = get_dataloaders(
        data_path="/home/userdata/2024_hyn/dataset/nasa_dashlink/pred.pt",
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)  # 每20轮lr减半
    ...
    with open(log_file, "w") as f:
        for epoch in range(1, config.epochs + 1):
            ...
            f.write(f"Epoch {epoch}: train_loss={train_loss:.6f}\n")
