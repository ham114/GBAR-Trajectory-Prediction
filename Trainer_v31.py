...
from models.PGM_v5 import VAEPrior
...
def classifier_probs(clf, batch, device):
    x = batch["geo_codes_bits"].to(device).float()
    logits = clf(x)
    logits = torch.softmax(logits, dim=-1)
    return logits  # [B,P], softmax 已在模型里做
...
def get_phase_w_mixed(batch, device, cfg, clf=None, step=None, total_steps=None, is_eval: bool=False):
    w_gt = batch["labels_onehot"].to(device).float()
